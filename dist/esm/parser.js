import { CodeString, LispType, SandboxCapabilityError, isLisp, reservedWords } from "./utils.js";
...
setLispType(["incrementerAfter"], (ctx) => {
	const { constants, part, res, expect } = ctx;
	if (isLisp(ctx.lispTree) && (ctx.lispTree[0] === LispType.Number || ctx.lispTree[0] === LispType.BigInt || ctx.lispTree[0] === LispType.GlobalSymbol || ctx.lispTree[0] === LispType.StringIndex || ctx.lispTree[0] === LispType.LiteralIndex || ctx.lispTree[0] === LispType.RegexIndex)) throw new SyntaxError("Invalid left-hand side expression in postfix operation");
	ctx.lispTree = lispify(constants, part.substring(res[0].length), expectTypes[expect].next, createLisp({
		op: incrementTypes["$" + res[0]],
		a: ctx.lispTree,
		b: LispType.None
...
var asyncOfStart2 = lispify({ maxDepth: 10 }, new CodeString("let $$iterator = $$obj"), ["initialize"]);
var asyncOfStart3 = lispify({ maxDepth: 10 }, new CodeString("let $$next = $$iterator.next()"), ["initialize"]);
var asyncOfStep = lispify({ maxDepth: 10 }, new CodeString("$$next = $$iterator.next()"));
