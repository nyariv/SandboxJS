...
function restOfExp(constants, part, tests, quote, firstOpening, closingsTests, details = {}, depth = 0) {
    ...
    if (depth > constants.maxDepth) {
        throw new utils.SandboxCapabilityError('Maximum expression depth exceeded');
    }
...
const ofStart2 = lispify({ maxDepth: 10 }, new utils.CodeString('let $$iterator = $$obj[Symbol.iterator]()'), ['initialize']);
const ofStart3 = lispify({ maxDepth: 10 }, new utils.CodeString('let $$next = $$iterator.next()'), ['initialize']);
const ofCondition = lispify({ maxDepth: 10 }, new utils.CodeString('return !$$next.done'), [
    'initialize',
]);
const ofStep = lispify({ maxDepth: 10 }, new utils.CodeString('$$next = $$iterator.next()'));
const inStart2 = lispify({ maxDepth: 10 }, new utils.CodeString('let $$keys = Object.keys($$obj)'), ['initialize']);
const inStart3 = lispify({ maxDepth: 10 }, new utils.CodeString('let $$keyIndex = 0'), [
    'initialize',
]);
const inStep = lispify({ maxDepth: 10 }, new utils.CodeString('$$keyIndex++'));
const inCondition = lispify({ maxDepth: 10 }, new utils.CodeString('return $$keyIndex < $$keys.length'), ['initialize']);
